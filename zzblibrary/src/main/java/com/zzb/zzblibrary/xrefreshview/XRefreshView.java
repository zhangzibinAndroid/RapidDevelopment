package com.zzb.zzblibrary.xrefreshview;
...
import com.zzb.zzblibrary.xrefreshview.callback.IFooterCallBack;
import com.zzb.zzblibrary.xrefreshview.callback.IHeaderCallBack;
import com.zzb.zzblibrary.xrefreshview.listener.OnBottomLoadMoreTime;
import com.zzb.zzblibrary.xrefreshview.listener.OnTopRefreshTime;
import com.zzb.zzblibrary.xrefreshview.utils.XRefreshUtils;
...
public class XRefreshView extends LinearLayout {
    ...
    private void dealAddHeaderView() {
        if (indexOfChild(mHeaderView) == -1) {
            XRefreshUtils.removeViewFromParent(mHeaderView);
    ...
    private void dealAddFooterView() {
        if (indexOfChild(mFooterView) == -1) {
            if (needAddFooterView()) {
                XRefreshUtils.removeViewFromParent(mFooterView);
                try {
...
//        if(mHolder.mOffsetY!=0)return;
                ...
                if (mHolder.hasHeaderPullDown()) {
                    ...
                    if (!mStopingRefresh) {
                        if (mEnablePullLoad && !isEmptyViewShowing() && needAddFooterView() && !mHasLoadComplete) {
                            ...
                            startScroll(offset, XRefreshUtils.computeScrollVerticalDuration(offset, getHeight()));
                        }
                    }
                }
                mLastY = -1; // reset
    ...
    public void setHeadMoveLargestDistence(int headMoveDistence) {
        if (headMoveDistence <= 0) {
            int ScreenHeight = XRefreshUtils.getScreenSize(getContext()).y;
    ...
    private void resetHeaderHeight() {
        ...
        if (mPullRefreshing) {
            ...
            startScroll(offsetY, XRefreshUtils.computeScrollVerticalDuration(offsetY, getHeight()));
        } else {
...
//            headerMovePercent = headerMovePercent > 1 ? 1 : headerMovePercent;
    ...
    protected void resetLayout() {
        ...
        if (mHolder.mOffsetY != 0 && !mStopingRefresh) {
            startScroll(-mHolder.mOffsetY, XRefreshUtils.computeScrollVerticalDuration(mHolder.mOffsetY, getHeight()));
        }
    }
...
//        mFooterCallBack.onStateRefreshing();
            ...
            if (mScroller.computeScrollOffset()) {
                ...
                if (mHolder.mOffsetY == 0) {
                    ...
                    if (mStopingRefresh && !mPullLoading && !mPullRefreshing) {
                        startScroll(-currentY, XRefreshUtils.computeScrollVerticalDuration(currentY, getHeight()));
                    }
                }
            }
        }
    ...
    public void setEmptyView(View emptyView) {
        XRefreshUtils.removeViewFromParent(emptyView);
