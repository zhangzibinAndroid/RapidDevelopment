...
import com.zzb.zzblibrary.xrefreshview.recyclerview.XSpanSizeLookup;
import com.zzb.zzblibrary.xrefreshview.utils.XRefreshUtils;
import com.zzb.zzblibrary.xrefreshview.view.XWebView;
import com.zzb.zzblibrary.xrefreshview.XRefreshView.XRefreshViewListener;
...
public class XRefreshContentView implements OnScrollListener, OnTopRefreshTime, OnBottomLoadMoreTime {
    ...
    public void onRecyclerViewScrolled(RecyclerView recyclerView, BaseRecyclerAdapter adapter, int dx, int dy, boolean force) {
        ...
        if (onRecyclerViewTop()) {
            if (XRefreshUtils.isRecyclerViewFullscreen(recyclerView)) {
//                        addFooterView(true);
            } else {
                if (mHideFooter) {
            ...
            if (!(manager instanceof GridLayoutManager)) {
                ...
                if (footerView != null) {
                    ...
                    if (layoutParams instanceof StaggeredGridLayoutManager.LayoutParams) {
                        XRefreshUtils.setFullSpan((StaggeredGridLayoutManager.LayoutParams) layoutParams);
...
//        mTotalItemCount = 0;
    ...
    private void dealRecyclerViewNotFullScreen() {
        RecyclerView recyclerView = (RecyclerView) child;
        if (onRecyclerViewTop() && !XRefreshUtils.isRecyclerViewFullscreen(recyclerView) && child instanceof RecyclerView && mFooterCallBack != null && isFooterEnable()) {
            mFooterCallBack.onStateReady();
            mFooterCallBack.callWhenNotAutoLoadMore(mParent);
            if (!mFooterCallBack.isShowing()) {
                mFooterCallBack.show(true);
